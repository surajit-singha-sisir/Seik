...
router.patch('/:id', async (req, res) => {
    try {
        const { albumId, favorite, hidden, archived } = req.body;
...
router.delete('/:id/tags/:tagId', async (req, res) => {
    try {
        await db.delete(fileTags).where(and(eq(fileTags.fileId, req.params.id), eq(fileTags.tagId, req.params.tagId)));
        res.json({ ok: true });
    }
    catch (err) {
...
router.post('/:id/tags', async (req, res) => {
    try {
        ...
        if (!resolvedTagId)
            return res.status(400).json({ error: 'tagId or tagName required.' });
