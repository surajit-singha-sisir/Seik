...
async function loadAllSettings() {
    ...
    for (const row of rows) {
        if (row.key in result || Object.values(SETTINGS_KEYS).includes(row.key)) {
...
router.patch('/env', async (req, res) => {
    try {
        ...
        if (databaseUrl !== undefined)
            toWrite.DATABASE_URL = databaseUrl;
        if (imgbbApiKey !== undefined)
            toWrite.IMGBB_API_KEY = imgbbApiKey;
        if (imgbbApiUrl !== undefined)
            toWrite.IMGBB_API_URL = imgbbApiUrl;
        if (maxUploadSizeMb !== undefined)
            toWrite.MAX_UPLOAD_SIZE_MB = String(maxUploadSizeMb);
