 ...
 */
export async function compressImage(buffer, mimeType, quality) {
    ...
    try {
        ...
        if (out.length >= buffer.length) {
            console.warn(`[compression] re-encoded output (${out.length}B) was not smaller than original (${buffer.length}B) ` +
